...
def main():
    ...
    #ips = [base_url.format(i+10) for i in range(2)]
    #(txs, d, mine) = ([], {'sender':ips[0], 'recipient':ips[1], 'amount':2}, [])
    #res = [test_client("http://localhost:5011/transactions/new", d)]
    #res = [test_client("http://localhost:5011/transactions/new", d)]
    #res.append(test_client("http://localhost:5010/transactions/new", d))
    #print([r[0][0].text for r in res])
    #exit()
    n_users = 5
